...
class wpda_contdown_extend_popup_page{
	...
	public static function save_in_db(){
		...
		if(isset($_POST['curent_page']) && isset($_POST['countdown_extended_options_nonce']) && wp_verify_nonce( $_POST['countdown_extended_options_nonce'],'countdown_extended_options_nonce')){
			if($_POST['curent_page']=="general_save_parametr"){
				update_option("wpdevart_countdown_extended_popup_enable",sanitize_text_field($_POST['countdown_extended_page_mode']));
			}else{
				foreach($params_array[$_POST['curent_page']]["params"] as $key => $value){				
					if(isset($_POST[$key])){
						$databases_parametrs[sanitize_text_field($key)]=stripslashes(sanitize_text_field($_POST[$key]));					
					}
					else{
						$kk=0;
	...
	public function controller_page(){
		...
		?>
		<script>
			var countdown_extended_ajaxurl="<?php echo esc_url(admin_url( 'admin-ajax.php')); ?>";
			var countdown_extended_plugin_url="<?php echo wpda_contdown_extend_plugin_url; ?>";
			var countdown_extended_parametrs_sucsses_saved="<?php echo esc_html(self::$text_parametrs['parametrs_sucsses_saved']); ?>";
		...
		<div class="countdown_title"><h1><span>Countdown Popup</span></h1></div>    

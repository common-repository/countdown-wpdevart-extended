...
/*############  Update theme ID function  ################*/
	...
	private function display_table_list_theme(){
        ...
        <div class="wrap">
			...
			<h2 style="width: 95%">Theme <a href="admin.php?page=wpda_contdown_extend_themes&task=add_wpda_contdown_extend_theme" class="add-new-h2">Add New</a></h2>            
            ...
            <table class="wp-list-table widefat fixed pages" style="width:95%">
                ...
                <tbody>
                 <tr ng-repeat="rows in names | filter:filtering_table" class="description_row">
					 ...
					 <td><a href="<?php echo urldecode(htmlspecialchars_decode(wp_nonce_url('admin.php?page=wpda_contdown_extend_themes&task=set_default_theme&id={{rows.id}}','wpda_nonce','wpda_nonce'))); ?>"><img src="<?php echo wpda_contdown_extend_plugin_url.'includes/admin/images/default' ?>{{rows.default}}.png"></a></td>
					 <td><a href="admin.php?page=wpda_contdown_extend_themes&task=add_edit_theme&id={{rows.id}}">Edit</a></td>
					 <td><a href="<?php echo urldecode(htmlspecialchars_decode(wp_nonce_url('admin.php?page=wpda_contdown_extend_themes&task=remove_theme&id={{rows.id}}','wpda_nonce','wpda_nonce'))); ?>">Delete</a></td>                               
                  </tr> 
                </tbody>
            </table>
        </form>
...
jQuery(document).ready(function(e) {
	...
	private function add_edit_theme($id=0){
		...
		<form action="admin.php?page=wpda_contdown_extend_themes<?php if($id) echo '&id='.esc_attr($id); ?>" method="post" name="adminForm" class="top_description_table" id="adminForm">
            <div class="conteiner">
                <div class="header">
                        ...
                        <div class="wpda_theme_link_tabs">
							<?php
								...
								foreach($this->options as $params_grup_name =>$params_group_value){ 
									echo '<li id="'.esc_attr($params_grup_name).'_tab">'.stripslashes(wp_filter_post_kses($params_group_value['heading_name'])).'</li>';
								}
								echo "</ul>";
							?>
						</div>
	...
	private function select_font_with_label($select_name,$main_value='',$bind=''){
		...
		<select class="wpda_gallselect" name="<?php echo 'parametrs['.esc_attr($select_name).']'; ?>" id="<?php echo esc_attr($select_name); ?>" >
		<?php
	...
	private function select_border_with_label($select_name,$main_value='',$bind=''){
		?>
		<select class="wpda_gallselect" name="<?php echo 'parametrs['.esc_attr($select_name).']'; ?>" id="<?php echo esc_html($select_name); ?>" >
		<?php

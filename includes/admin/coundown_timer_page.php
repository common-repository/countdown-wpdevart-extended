	...
	public static function return_params_array(){	
		return array(
			"timer_set_time"=>array(
				...
				"params"=>array(
					...
					"timer_start_time"=>array(
						...
						"small_text"=>"(Date)",
						"default_value"=>date('d/m/Y', (time()))." 00:00",
					),
					...
					"timer_coundown_repeat"=>array(
						"title"=>"Countdown Repeat",
						"description"=>"Countdown Repeat type",				
						"function_name"=>"simple_select",
						"values"=>array("none"=>"Dont Repeat","when_end"=>"Repeat when end","daily"=>"Daily"),
						"default_value"=>"none",
						"pro"=>true,
					),
				)
			...
			"after_timer"=>array(
				...
				"params"=>array(					
					"after_countdown_end_type"=>array(
						...
						"function_name"=>"simple_select",
						"values"=>array("hide"=>"Hide Countdown","text"=>"Show Text","redirect"=>"Redirect"),
						"default_value"=>"hide",
					),
			...
			"timer_text"=>array(
				...
				"aditional_heading_name"=>"Countodwn top and bottom texts",
				"params"=>array(					
...
/*############  Save function  ################*/
	...
	private function save_timer(){
		...
		if(!wp_verify_nonce($_REQUEST['wpda_countdown_extended_nonce'],'wpda_countdown_extended_nonce')){
			return;
		}
		$params_array=array();
		if(isset($_POST['name'])){
		...
		$params_array=array('name'=>sanitize_text_field($name),"version"=>"1.2");
		foreach($this->options as $param_heading_key=>$param_heading_value){
			foreach($param_heading_value['params'] as $key=>$value){
				if(isset($_POST[$key])){
					if(is_array($_POST[$key])){						
						$loc_array=[];
						foreach($_POST[$key] as $loc_array_key => $loc_array_value ){
							$sanitize_key=sanitize_text_field($loc_array_key);
							$loc_array[$sanitize_key]=sanitize_text_field($loc_array_value);
						}
						$params_array[$key]=$loc_array;
					}else{
		...
		$save_or_no=$wpdb->insert( wpda_contdown_extend_databese::$table_names['timer'], 
			array( 
				'name' => $name==''?'Unnamed':sanitize_text_field( $name ),
...
/*############  Update timer ID function  ################*/
	...
	private function remove_timer($id){
		if(!wp_verify_nonce($_REQUEST['wpda_nonce'],'wpda_nonce')){
			return;
		}	
		global $wpdb;
	...
	private function display_table_list_timer(){
        ...
        <div class="wrap">
			...
			<h2 style="width: 95%">Timer <a href="admin.php?page=wpda_contdown_extend_menu&task=add_wpda_contdown_extend_timer" class="add-new-h2">Add New</a></h2>            
            ...
            <table class="wp-list-table widefat fixed pages" style="width:95%">
                ...
                <tbody>
                 <tr ng-repeat="rows in names | filter:filtering_table" class="description_row">
					 ...
					 <td><a href="<?php echo urldecode(htmlspecialchars_decode(wp_nonce_url('admin.php?page=wpda_contdown_extend_menu&task=remove_timer&id={{rows.id}}','wpda_nonce','wpda_nonce'))); ?>">Delete</a></td>                               
                  </tr> 
                </tbody>
            </table>
        </form>
...
jQuery(document).ready(function(e) {
	...
	private function add_edit_timer($id=0){
		...
		?>	
			<div class="add_edit_support wpdevart_plugins_header div-for-clear">
				<div class="wpdevart_plugins_get_pro div-for-clear">
					<div class="wpdevart_plugins_get_pro_info">
						<h3>WpDevArt Countdown Extended Premium</h3>
						<p>Powerful and Customizable Countdown Timer</p>
		...
		<form action="admin.php?page=wpda_contdown_extend_menu<?php if($id) echo '&id='.esc_attr($id); ?>" method="post" name="adminForm" class="top_description_table" id="adminForm">
            <div class="conteiner">
                <div class="header">
                    <span><h2 class="wpda_timer_title"><?php if($id) {echo "Edit Timer ".(isset($name)?'<span style="color:#7052fb">'.esc_html($name).'</span>':'');}else{echo "Add Timer";} ?></h2></span>
                ...
                <div class="option_panel">            
                    ...
                    <div class="all_options_panel">
                        <input type="text" class="timer_name" name="name" placeholder="Enter name here" value="<?php echo isset($name)?esc_html($name):'' ?>" >
						...
						<?php 
						foreach($this->options as $params_grup_name =>$params_group_value){ 
							wpda_contdown_extend_library::create_table_heading((isset($params_group_value['aditional_heading_name'])?$params_group_value['aditional_heading_name']:$params_group_value['heading_name']),$params_grup_name);
					...
					<?php wp_nonce_field('wpda_countdown_extended_nonce','wpda_countdown_extended_nonce'); ?>
                    </div>
                </div>
            </div>
		</form>
	...
	private function select_font_with_label($select_name,$main_value='',$bind=''){
		...
		<select class="wpda_gallselect" name="<?php echo 'parametrs['.esc_html($select_name).']'; ?>" id="<?php echo esc_attr($select_name); ?>" >
		<?php
		
		foreach($this->fonts_options() as $key => $value){
			?>
			<option <?php selected($key,$main_value) ?> value="<?php echo esc_attr($key); ?>" ><?php echo esc_attr($value); ?></option>
			<?php 					
		}
		?>
		</select>																
	...
	private function select_border_with_label($select_name,$main_value='',$bind=''){
		...
		foreach($this->border_types() as $key => $value){
			?>
			<option <?php selected($key,$main_value) ?> value="<?php echo esc_attr($key); ?>"><?php echo esc_html($value); ?></option>
			<?php 					
		}
		?>
		</select>																

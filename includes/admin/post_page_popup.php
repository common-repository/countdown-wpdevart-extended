...
class wpda_countdown_post_page_popup{	
	...
	private function required_js_and_style(){
		...
		?>
		<script type="text/javascript" src="<?php echo wpda_contdown_extend_plugin_url ?>includes/admin/js/post_page_popup.js"></script>
                ...
                <tbody>
                    <tr>
                        ...
                        <td class="paramlist_value" >                        
							...
							<div><a target="_blank" style="text-decoration: none; color: #5b9dd9; font-size: 14px;" href="<?php echo esc_url(get_admin_url()).'admin.php?page=wpda_contdown_extend_menu&task=add_wpda_contdown_extend_timer'  ?>">(Add new timer)</a></div>      
                        </td>
                    </tr>
                    <tr>
	...
	private function print_timers(){
		...
		foreach($timers as $timer){
			?><option <?php selected($timer_id,$timer->id); ?> value="<?php echo esc_attr($timer->id); ?>"><?php echo esc_html($timer->name) ?></option><?php 
		}
	}
	private function print_themes(){
		...
		foreach($themes as $theme){
			?><option <?php selected($theme_id,$theme->id); ?> value="<?php echo esc_attr($theme->id); ?>"><?php echo esc_html($theme->name); ?></option><?php 
		}
	}
}
?>

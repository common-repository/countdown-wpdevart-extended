...
// wpda_contdown_extend_plugin_path
	...
	public function include_scripts(){
		...
		wp_enqueue_script('wpdevart_countdown_extendex_standart',wpda_contdown_extend_plugin_url.'includes/frontend/js/front_timer.js');
	...
	private function woocommerce_front_filters(){
		add_filter('woocommerce_single_product_summary',array($this,'woocomerce_front_end_single'),11);			
	}
	...
	public function woocomerce_front_end_single(){
		$prod_id=get_the_ID();
		$enable = get_post_meta( $prod_id, 'wpda_countdown_extended_enable', true );
		if($enable === '0'){
			return '';
		}
		$timer = get_post_meta( $prod_id, 'wpda_countdown_extended_timer', true );
		$theme = get_post_meta( $prod_id, 'wpda_countdown_extended_theme', true );
		if($timer!= '0')

...
class wpdevart_countdown_extended extends WP_Widget {
	...
	function form($instance) {
        ...
        <p class="flb_field">
          ...
          <br>
          <input id="<?php echo esc_html($this->get_field_id('title')); ?>" name="<?php echo esc_html($this->get_field_name('title')); ?>" type="text" value="<?php echo esc_html($instance['title']); ?>" class="widefat">
        </p>
        
        <p class="flb_field">  

			<select style="width: 100%" id="<?php echo esc_html($this->get_field_id('timer')); ?>" name="<?php echo esc_html($this->get_field_name('timer')); ?>">
				<option value="0">Select Timer</option>
				<?php foreach($timers as $id => $name){
					echo "<option ".selected($id,$instance['timer'],false)." value=\"".esc_html($id)."\">".esc_html($name)."</option>";
				} ?>
			</select>  
            <a target="_blank" style="text-decoration: none; color: #5b9dd9; font-size: 14px;" href="<?php echo esc_url(get_admin_url()).'admin.php?page=wpda_contdown_extend_menu&task=add_wpda_contdown_extend_timer'  ?>">(Add new timer)</a>   
        </p>
        
        <p class="flb_field">  
			<select style="width: 100%" id="<?php echo esc_html($this->get_field_id('theme')); ?>" name="<?php echo esc_html($this->get_field_name('theme')); ?>">
				<option value="0">Select Theme</option>
				<?php foreach($themes as $id => $name){
					echo "<option ".selected($id,$instance['theme'],false)." value=\"".esc_html($id)."\">".esc_html($name)."</option>";
				} ?>
			</select>  
        </p>
...
/*############################### Front-end function ###############################################*/	
	...
	private function wpdevart_generete_front_end($parametrs){
		...
		return do_shortcode('[wpda_contdown_extend timer_id="'.esc_html($parametrs["timer"]).'"  theme_id="'.esc_html($parametrs["theme"]).'"]');
	}
}